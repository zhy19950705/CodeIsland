...
actor CodexAppServerClient {
    ...
    private let logger = Logger(subsystem: "com.codeisland", category: "CodexAppServer")
    ...
    private func postPermissionRequest(threadId: String, toolName: String, prompt: String?, toolInput: [String: String]?) async {
        await MainActor.run {
            ...
            NotificationCenter.default.post(name: .codeIslandCodexPermissionRequested, object: nil, userInfo: userInfo)
        }
    }
    ...
    private func postQuestionRequest(
        ...
        await MainActor.run {
            ...
            NotificationCenter.default.post(name: .codeIslandCodexQuestionRequested, object: nil, userInfo: userInfo)
        }
    }
    ...
    private func postThreadRefreshRequested(threadId: String) async {
        await MainActor.run {
            NotificationCenter.default.post(
                name: .codeIslandCodexThreadRefreshRequested,
                object: nil,
...
extension Notification.Name {
    static let codeIslandCodexPermissionRequested = Notification.Name("CodeIslandCodexPermissionRequested")
    static let codeIslandCodexQuestionRequested = Notification.Name("CodeIslandCodexQuestionRequested")
    static let codeIslandCodexThreadRefreshRequested = Notification.Name("CodeIslandCodexThreadRefreshRequested")
}
