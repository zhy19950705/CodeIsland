...
actor DiagnosticsExporter {
    ...
    func exportArchive(
        ...
        let copiedFiles: [(source: URL, relativePath: String)] = [
            (fileManager.homeDirectoryForCurrentUser.appendingPathComponent(".superisland/sessions.json"), "state/sessions.json"),
    ...
    private func writeUnifiedLogs(to destinationURL: URL) async throws {
        try await writeCommandOutput(
            ...
            arguments: ["show", "--last", "6h", "--predicate", "subsystem == \"com.superisland\""],
            to: destinationURL
        )
    }
